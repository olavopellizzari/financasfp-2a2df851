...
export function ReportsPage() {
  const { allTransactions, categories, allAccounts, allCards, getCategoryById } = useFinance();
  ...
  // Estados para o Explorador Dinâmico
  const [explorerMetric, setExplorerMetric] = useState<'expense' | 'income' | 'card'>('expense');
  const [explorerCategoryId, setExplorerCategoryId] = useState<string>('all');
  const [explorerCardId, setExplorerCardId] = useState<string>('all');
  ...
  const annualData = useMemo(() => {
    ...
    const baseData = months.map((month, index) => {
      ...
      return { month: format(month, 'MMM', { locale: ptBR }), monthFull: format(month, 'MMMM', { locale: ptBR }), income, expenses: netExpense, balance: monthBalance, accumulated: accumulatedBalance, isFuture: index > currentMonthIdx };
    });

    const pastMonthsWithData = baseData.filter((m, idx) => idx <= currentMonthIdx && (m.income > 0 || m.expenses > 0));
    const avgMonthlyBalance = pastMonthsWithData.length > 0 ? pastMonthsWithData.reduce((sum, m) => sum + m.balance, 0) / pastMonthsWithData.length : 0;
    let projectedAccumulated = baseData[Math.max(0, Math.min(currentMonthIdx, 11))]?.accumulated || 0;
    
    return baseData.map((m, idx) => {
      if (idx <= currentMonthIdx) return { ...m, projected: m.accumulated };
  ...
  const annualTotals = useMemo(() => {
    const totals = annualData.reduce((acc, m) => ({ income: acc.income + m.income, expenses: acc.expenses + m.expenses, balance: acc.balance + m.balance }), { income: 0, expenses: 0, balance: 0 });
    ...
    return { ...totals, avgMonthlyExpense: totals.expenses / activeCount, bestMonth: sortedMonths[sortedMonths.length - 1] || null, worstMonth: sortedMonths[0] || null, activeCount };
  }, [annualData]);
  ...
  const explorerData = useMemo(() => {
    ...
    return months.map(month => {
      const monthStr = format(month, 'yyyy-MM');
      let txs = filteredSource.filter(t => t.effectiveMonth === monthStr && t.status !== 'cancelled');

      if (explorerMetric === 'income') {
        ...
        if (explorerCardId !== 'all') txs = txs.filter(t => t.cardId === explorerCardId);
      }

      const amount = txs.reduce((sum, t) => t.type === 'REFUND' ? sum - t.amount : sum + t.amount, 0);
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.setFontSize(20); doc.setTextColor(34, 197, 94); doc.text("Relatório Financeiro Anual", 14, 22);
      doc.setFontSize(10); doc.setTextColor(100); doc.text(`Ano: ${selectedYear} | Filtro: ${userName}`, 14, 30);
      const summaryData = [["Total de Receitas", formatCurrency(annualTotals.income)], ["Total de Despesas", formatCurrency(annualTotals.expenses)], ["Saldo Acumulado", formatCurrency(annualTotals.balance)], ["Média Mensal", formatCurrency(annualTotals.avgMonthlyExpense)]];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <UserFilter value={selectedUserId} onChange={setSelectedUserId} showTotalOption={true} className="w-[200px]" />
        </div>
      </div>
      ...
      <Tabs defaultValue="monthly" className="w-full">
        <TabsList className="grid w-full grid-cols-3 max-w-[600px] mb-6">
          ...
          <TabsTrigger value="explorer" className="gap-2"><Search className="h-4 w-4" /> Explorador</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="monthly" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2">
              <CardHeader><CardTitle className="text-lg flex items-center gap-2"><Zap className="h-5 w-5 text-primary" /> Gastos Diários</CardTitle></CardHeader>
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                <div className="space-y-2"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Gastos Fixos</span><span className="font-bold">{formatCurrency(monthlyStats.fixed)}</span></div><div className="h-2 bg-muted rounded-full overflow-hidden"><div className="h-full bg-primary" style={{ width: `${(monthlyStats.fixed / (monthlyStats.expenses || 1)) * 100}%` }} /></div></div>
                <div className="space-y-2"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Gastos Variáveis</span><span className="font-bold">{formatCurrency(monthlyStats.variable)}</span></div><div className="h-2 bg-muted rounded-full overflow-hidden"><div className="h-full bg-orange-500" style={{ width: `${(monthlyStats.variable / (monthlyStats.expenses || 1)) * 100}%` }} /></div></div>
                <div className="pt-4 border-t"><p className="text-xs font-bold text-muted-foreground uppercase mb-3">Top 5 Estabelecimentos</p><div className="space-y-3">{monthlyStats.topMerchants.map((m, i) => (<div key={i} className="flex items-center justify-between text-sm"><span className="truncate max-w-[150px]">{m.name}</span><span className="font-bold">{formatCurrency(m.value)}</span></div>))}</div></div>
              </CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="explorer" className="space-y-6">
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={explorerMetric} onValueChange={(v: any) => setExplorerMetric(v)}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="expense">Despesas</SelectItem>
                      <SelectItem value="income">Receitas</SelectItem>
                      <SelectItem value="card">Gastos no Cartão</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                {explorerMetric !== 'card' ? (
                  <div className="space-y-2">
                    ...
                    <Select value={explorerCategoryId} onValueChange={setExplorerCategoryId}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">Todas as Categorias</SelectItem>
                        {categories
                          .filter(c => explorerMetric === 'income' ? c.type === 'income' : c.type === 'expense')
                          .map(cat => (
                  ...
                  <div className="space-y-2">
                    <Label>Cartão</Label>
                    <Select value={explorerCardId} onValueChange={setExplorerCardId}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="flex items-end">
                  <div className="p-3 bg-primary/5 rounded-xl border border-primary/10 w-full">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase">Total no Período</p>
                    <p className="text-lg font-bold text-primary">
                      {formatCurrency(explorerData.reduce((sum, d) => sum + d.amount, 0))}
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <Tooltip 
                      content={({ active, payload }) => {
                        if (active && payload && payload.length) {
                          return (
                            <div className="bg-card border shadow-xl p-3 rounded-xl">
                              <p className="text-xs font-bold mb-1">{payload[0].payload.name} {selectedYear}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card>
              <CardHeader><CardTitle className="text-sm font-bold flex items-center gap-2"><Tags className="h-4 w-4" /> Categorias Mais Usadas</CardTitle></CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader><CardTitle className="text-sm font-bold flex items-center gap-2"><CreditCard className="h-4 w-4" /> Uso de Cartões</CardTitle></CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {allCards.filter(c => !c.is_archived).map(card => {
                    const amount = filteredSource
                      .filter(t => t.cardId === card.id && t.status !== 'cancelled' && t.effectiveMonth.startsWith(selectedYear.toString()))
                      .reduce((sum, t) => t.type === 'REFUND' ? sum - t.amount : sum + t.amount, 0);
                    return (
                      <div key={card.id} className="flex items-center justify-between">
                        <span className="text-sm font-medium">{card.name}</span>
                        <span className="text-sm font-bold">{formatCurrency(Math.max(0, amount))}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <Card className="bg-primary/5 border-primary/20">
              <CardHeader><CardTitle className="text-sm font-bold flex items-center gap-2"><Activity className="h-4 w-4" /> Insights do Ano</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="p-3 bg-card rounded-xl border">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase">Média Mensal de Gastos</p>
                  <p className="text-lg font-bold">{formatCurrency(annualTotals.avgMonthlyExpense)}</p>
                </div>
                <div className="p-3 bg-card rounded-xl border">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase">Mês com Maior Economia</p>
                  <p className="text-lg font-bold text-income">{annualTotals.bestMonth?.monthFull || '---'}</p>
                </div>
              </CardContent>
            </Card>
          </div>
