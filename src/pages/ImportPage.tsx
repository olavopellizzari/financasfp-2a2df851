...
export function ImportPage() {
  ...
  const suggestCategory = useCallback((description: string, transactionType: TransactionType): string => {
    ...
    if (transactionType === 'INCOME' || transactionType === 'REFUND') {
      normalizedCategoryMap.forEach((cat, name) => { if (cat.type === 'income' || cat.kind === 'receita') relevantCategoryNames.add(name); });
    } else {
      normalizedCategoryMap.forEach((cat, name) => { if (cat.type === 'expense' || cat.kind === 'despesa') relevantCategoryNames.add(name); });
    }
    for (const [keyword, catNameNormalized] of Object.entries(CATEGORY_KEYWORDS)) {
      if (desc.includes(keyword)) {
        if (relevantCategoryNames.has(catNameNormalized)) { return normalizedCategoryMap.get(catNameNormalized)?.id || ''; }
      }
    }
    const defaultCatName = (transactionType === 'INCOME' || transactionType === 'REFUND') ? 'outras receitas' : 'outras despesas';
    if (relevantCategoryNames.has(defaultCatName)) { return normalizedCategoryMap.get(defaultCatName)?.id || ''; }
    return '';
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 2; i < rows.length; i++) {
        ...
        let date: Date;
        if (typeof cols[0] === 'number') { date = new Date((cols[0] - 25569) * 86400 * 1000 + (12 * 3600 * 1000)); }
        else {
          const dateParts = cols[0].toString().split('/');
          if (dateParts.length === 3) { date = new Date(parseInt(dateParts[2]), parseInt(dateParts[1]) - 1, parseInt(dateParts[0]), 12, 0, 0); }
          else { date = new Date(cols[0]); date.setHours(12, 0, 0, 0); }
        }
        ...
        result.push({ date, description, amount, userName, type: transactionType, matchedUserId, targetAccountId, suggestedCategoryId, installments });
      }
      setParsedData(result);
      toast({ title: 'Arquivo processado!', description: `${result.length} itens encontrados.` });
    } catch (error) { toast({ title: 'Erro', description: 'Falha ao processar arquivo', variant: "destructive" }); } finally { setIsLoading(false); }
  };
    ...
    try {
      for (const item of parsedData) {
        ...
        if (importType === 'card' && finalCardId) {
          ...
          firstMesFaturaDate = parse(firstMesFaturaStr, 'yyyy-MM', new Date());
        }
      ...
      setFileName('');
      await refresh();
    } catch (error) { toast({ title: 'Erro ao importar', variant: "destructive" }); } finally { setIsLoading(false); }
  };

  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-bold">Importar Lançamentos</h1><p className="text-muted-foreground">Importe gastos do cartão ou débito via Excel (.xlsx) ou CSV</p></div>
      <Card><CardContent className="pt-6 space-y-6"><Tabs value={importType} onValueChange={(v: any) => { setImportType(v); setGlobalTargetId(''); setParsedData([]); }}><TabsList className="grid w-full grid-cols-2 max-w-md"><TabsTrigger value="card" className="gap-2"><CreditCard className="h-4 w-4" /> Cartão de Crédito</TabsTrigger><TabsTrigger value="account" className="gap-2"><Wallet className="h-4 w-4" /> Conta (Débito/Carnê)</TabsTrigger></TabsList></Tabs><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{importType === 'card' ? (<div className="space-y-2"><Label>Cartão de Destino</Label><Select value={globalTargetId} onValueChange={setGlobalTargetId}><SelectTrigger><SelectValue placeholder="Selecione o cartão" /></SelectTrigger><SelectContent>{allCards.filter(c => !c.is_archived).map(card => (<SelectItem key={card.id} value={card.id}>{card.name}</SelectItem>))}</SelectContent></Select></div>) : (<div className="p-4 bg-muted/50 rounded-lg border border-dashed flex items-center gap-3"><div className="p-2 bg-primary/10 rounded-full"><Wallet className="h-5 w-5 text-primary" /></div><div className="text-sm"><p className="font-bold">Seleção Automática</p><p className="text-muted-foreground">Lançamentos na <strong>Conta Corrente</strong> de cada usuário.</p></div></div>)}<div className="flex items-end"><Button variant="outline" onClick={() => fileInputRef.current?.click()} className="w-full h-10"><FileSpreadsheet className="mr-2 h-4 w-4" /> {fileName || 'Selecionar Excel ou CSV'}</Button><input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleFileSelect} className="hidden" /></div></div>{headerMonth && (<div className="flex items-center gap-2 p-3 bg-primary/5 rounded-lg border border-primary/10"><Calendar className="h-4 w-4 text-primary" /><span className="text-sm font-medium">Mês de Lançamento: <strong className="capitalize">{format(parse(headerMonth, 'yyyy-MM', new Date()), 'MMMM yyyy', { locale: ptBR })}</strong></span></div>)}</CardContent></Card>
      {parsedData.length > 0 && (<Card><CardHeader><CardTitle className="text-lg">Prévia ({parsedData.length} itens)</CardTitle></CardHeader><CardContent><div className="space-y-2 max-h-96 overflow-y-auto">{parsedData.map((item, index) => (<div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg text-sm"><div className="flex flex-col"><div className="flex items-center gap-2"><span className="font-bold">{item.description}</span></div><div className="text-xs text-muted-foreground">{format(item.date, 'dd/MM/yyyy')}</div></div><div className="flex items-center gap-4"><Badge variant="outline" className="h-5 text-[10px]">{users.find(u => u.id === item.matchedUserId)?.name || item.userName}</Badge><Badge variant="secondary" className="h-5 text-[10px]">{categories.find(c => c.id === item.suggestedCategoryId)?.name || 'Sem Categoria'}</Badge><span className={cn("font-bold", item.type === 'REFUND' ? "text-income" : "text-expense")}>R$ {item.amount.toFixed(2)}</span></div></div>))}</div><Button onClick={handleImport} className="w-full mt-4 gradient-primary" disabled={isLoading}>Confirmar Importação</Button></CardContent></Card>)}
    </div>
  );
}