...
export function GoalsPage() {
  ...
  const { allAccounts, getAccountBalance, goals, saveGoal, deleteGoal } = useFinance();
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      await saveGoal({
        ...
        is_completed: currentBalance >= parseFloat(goalForm.targetAmount)
      });
              ...
              <div className="flex items-stretch h-full">
                ...
                <div className="flex-1 p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ backgroundColor: `${goal.color}20` }}>{goal.icon}</div>
                      <div>
                        <h3 className="font-bold flex items-center gap-2">{goal.name} {isCompleted && <CheckCircle className="h-4 w-4 text-income" />}</h3>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider">{goal.deadline ? `Prazo: ${safeFormatDate(goal.deadline, 'dd/MM/yy')}` : 'Sem prazo'}</p>
                      </div>
                    </div>
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEditGoal(goal)}><Pencil className="h-4 w-4" /></Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleDeleteGoal(goal.id)}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm"><span className="font-medium">{formatCurrency(currentBalance)}</span><span className="text-muted-foreground">{formatCurrency(goal.target_amount)}</span></div>
                    <Progress value={Math.min(percentage, 100)} className="h-2" />
                    <div className="flex justify-between items-center"><span className="text-xs text-muted-foreground">{isCompleted ? 'Meta alcançada!' : `Faltam ${formatCurrency(Math.max(0, goal.target_amount - currentBalance))}`}</span><span className="text-sm font-bold" style={{ color: goal.color }}>{percentage.toFixed(0)}%</span></div>
                  </div>
                </div>
              </div>
            </Card>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild><Button variant="outline" className={cn("w-full justify-start text-left font-normal", !goalForm.deadline && "text-muted-foreground")}><CalendarIcon className="mr-2 h-4 w-4" />{goalForm.deadline ? format(goalForm.deadline, "dd/MM/yyyy") : "Selecione uma data"}</Button></PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={goalForm.deadline || undefined} onSelect={(date) => setGoalForm(prev => ({ ...prev, deadline: date || null }))} disabled={(date) => date < new Date()} initialFocus /></PopoverContent>
              </Popover>
            </div>
            <div className="space-y-2">
              <Label>Ícone</Label>
